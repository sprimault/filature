...
// commande traite les sous-commandes qui ne lancent pas de partie, et dit si
...
// n'existe plus.
func commande(sortie io.Writer, nom, argument, dossierPlugins string) (traitee bool, err error) {
	switch nom {
	case "":
	...
	case "examples":
		return true, extraire(argument, dossierPlugins)

	case "validate":
		return true, valider(sortie, argument)
...
// dans commande et dans executer.
func main() {
	...
	traitee, err := commande(os.Stdout, flag.Arg(0), flag.Arg(1), *dossierPlugins)
	if err != nil {
	...
	if err := executer(*heberger, *rejoindre, *dossierPlugins, *partie); err != nil {
		fmt.Fprintln(os.Stderr, "filature:", err)
		os.Exit(1)
	}
}
...
// valider contrôle un plugin avant qu'il soit installé, et affiche son
...
func valider(sortie io.Writer, dossier string) error {
	if dossier == "" {
		return errors.New("usage: filature validate <dossier>")
	}
	if err := loader.Valider(dossier); err != nil {
		return err
	}

	somme, err := loader.Empreinte(dossier)
	if err != nil {
		return err
	}
...
func executer(heberger bool, rejoindre, dossierPlugins, partie string) error {
	return fmt.Errorf("à implémenter : étape 5")
}

// extraire écrit les plugins livrés dans un dossier, pour servir de modèle.
//
...
func extraire(dossier, actifs string) error {
	...
	return fs.WalkDir(plugins.Livres(), ".", func(chemin string, e fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
		...
		contenu, err := fs.ReadFile(plugins.Livres(), chemin)
		if err != nil {
			return err
		}
