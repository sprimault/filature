...
// troisCoups rend une list courte mais variée, de quoi exercer la description
// comme la sélection.
func troisCoups() []core.Move {
	return []core.Move{
...
func TestQuitAndEndOfInput(t *testing.T) {
	for nom, entree := range map[string]string{
		...
		t.Run(nom, func(t *testing.T) {
			_, err := ReadMove(strings.NewReader(entree), &strings.Builder{}, troisCoups())
...
func TestDescribeEveryMoveType(t *testing.T) {
	cas := []struct {
		...
		{core.Move{Type: core.MoveAbility, Piece: 2, Ability: "traqueur"}, "capacité traqueur de C"},
		{core.Move{Type: core.MoveExpense, Expense: core.ExpenseMurder}, "dépenser meurtre"},
...
// TestMovesNumbered vérifie que la list affichée et la saisie s'accordent sur
// le premier numéro.
func TestMovesNumbered(t *testing.T) {
	list := Moves(troisCoups())
