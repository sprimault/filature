...
// langueDeBase est le greffon qui porte le français, langue de repli.
...
func TestLanguesLivreesCouvrentLesCles(t *testing.T) {
	...
	dossiers, err := os.ReadDir(filepath.Join(racine, "greffons"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, d := range dossiers {
		...
		chemin := filepath.Join(racine, "greffons", d.Name(), "langue.toml")
...
// lireDictionnaire charge les libellés d'un greffon livré.
func lireDictionnaire(t *testing.T, greffon string) map[string]string {
	t.Helper()
	var d dictionnaire
	if _, err := toml.DecodeFile(filepath.Join(racine, "greffons", greffon, "langue.toml"), &d); err != nil {
		t.Fatalf("lecture de %s : %v", greffon, err)
	}
	return d.Libelle
}
