...
// majSchemas réécrit les schémas générés au lieu de les comparer.
//
...
func TestSchemaVueSuitLaStructure(t *testing.T) {
	attendu, err := schema.Generer(
		reflect.TypeOf(noyau.Vue{}),
		"https://github.com/sprimault/filature/schemas/vue.schema.json",
		"Vue Filature",
		"Ce qu'un camp a le droit de savoir. Le jeu n'expose rien d'autre, ni à l'interface, ni au reseau, ni a un bot. Genere depuis noyau.Vue : ne pas modifier a la main.",
	...
	if string(publie) != string(attendu) {
		t.Error("schemas/vue.schema.json ne correspond plus à noyau.Vue — " +
