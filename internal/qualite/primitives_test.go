...
//
// Le chargeur de greffons s'en sert pour refuser ce qu'il ne sait pas
...
func TestVocabulaireEnumereEnEntier(t *testing.T) {
	fset := token.NewFileSet()
	chemin := filepath.Join(racine, "internal", "noyau", "effets.go")

	var types []string
	for _, e := range noyau.TypesEffets() {
		types = append(types, string(e))
	}
	comparer(t, "TypesEffets", constantesDuType(t, fset, chemin, "TypeEffet"), types)

	var cibles []string
	for _, c := range noyau.Cibles() {
