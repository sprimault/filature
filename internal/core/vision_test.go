...
// list de coordonnées.
func plateauDessine(t *testing.T, croquis string) *BoundedBoard {
	t.Helper()
...
func TestSightWithoutRangeIsEmpty(t *testing.T) {
	b, _, err := Generate(1, parametresPour(21))
	if err != nil {
		t.Fatal(err)
	}
...
// table.
//
// La vue sert à afficher ce qu'un sideName couvre : y faire figurer des murs
// gonflerait la vue filtrée d'un tiers de cases sans rien apprendre.
func TestSightIgnoresBuildings(t *testing.T) {
	b, _, err := Generate(7, parametresPour(21))
	if err != nil {
		t.Fatal(err)
	}
...
func TestSightIsSymmetric(t *testing.T) {
	b, _, err := Generate(3, parametresPour(21))
	if err != nil {
		t.Fatal(err)
	}
...
func TestViewAndSpottingAgree(t *testing.T) {
	params := parametresPour(21)
	p, err := NewGame(ouvert(params.Size), 1, params, registreDEssai())
	if err != nil {
		t.Fatal(err)
	}
