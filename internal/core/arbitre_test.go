...
func partieAvecZone(fugitif Position, inspecteurs ...Position) *Game {
	b := grid(".....", ".....", ".....", ".....", ".....")
	b.zones = []Zone{
...
func TestExtractionTakesTwoTurns(t *testing.T) {
	p := partieAvecZone(Position{Column: 4, Row: 4})
...
func TestInspectorNeutralisesZone(t *testing.T) {
	p := partieAvecZone(Position{Column: 4, Row: 4}, Position{Column: 0, Row: 4})
...
func TestStaminaSpent(t *testing.T) {
	p := partieAvecZone(Position{Column: 2, Row: 2})
...
// ne se constate qu'en début de phaseName du fugitif.
...
func TestCorneredDetectedOnHandover(t *testing.T) {
	...
	b.zones = []Zone{{Number: 1, Cells: []Position{{Column: 0, Row: 0}}}}
	p := partieSur(b, Position{Column: 2, Row: 1})
