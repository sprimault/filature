...
// parametresDEssai réduit le plateau sans sortir de ce que Validate accepte.
func parametresDEssai() Settings {
	p := DefaultSettings()
	p.Size = 15
	p.Range = 7
	return p
...
func TestNewGameRejectsUnplayable(t *testing.T) {
	...
	cas := []struct {
		...
		{"paramètres refusés", ouvert(15), Settings{}, registreDEssai()},
	}
...
func TestNewGamePlacesFugitiveAtCentre(t *testing.T) {
	...
	for graine := int64(0); graine < 50; graine++ {
		p, err := NewGame(ouvert(params.Size), graine, params, registreDEssai())
		if err != nil {
			t.Fatal(err)
		}
...
func TestNewGameIsDeterministic(t *testing.T) {
	...
	a, err := NewGame(ouvert(params.Size), 178342119, params, registreDEssai())
	if err != nil {
		t.Fatal(err)
	}
	b, err := NewGame(ouvert(params.Size), 178342119, params, registreDEssai())
	if err != nil {
		t.Fatal(err)
	}
	...
	autre, err := NewGame(ouvert(params.Size), 1, params, registreDEssai())
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewGameSealsNoZone(t *testing.T) {
	params := parametresDEssai()
	p, err := NewGame(ouvert(params.Size), 7, params, registreDEssai())
	if err != nil {
		t.Fatal(err)
	}
...
func TestFullGameUndoes(t *testing.T) {
	params := parametresDEssai()
	depart, err := NewGame(ouvert(params.Size), 3, params, registreDEssai())
	if err != nil {
		t.Fatal(err)
	}
	avant := *depart
