...
// Step renvoie la case voisine dans la direction donnée, sans vérifier
// qu'elle est praticable : c'est le rôle de Game.LegalMoves.
func (p Position) Step(d Direction) Position {
	v := deplacements[d]
	return Position{p.Column + v.Column, p.Row + v.Row}
}

// Key rend la position sous une forme utilisable en clé JSON.
//
...
func (p Position) Key() string {
	return strconv.Itoa(p.Column) + "," + strconv.Itoa(p.Row)
}

// PositionFromKey relit ce que Key a écrit, et dit si la chaîne en était une.
//
...
func PositionFromKey(cle string) (Position, bool) {
	...
	return Position{Column: c, Row: l}, true
}

// DirectionTo renvoie la direction d'un pas entre deux cases adjacentes, et
...
func DirectionTo(de, vers Position) (Direction, bool) {
	ecart := Position{Column: vers.Column - de.Column, Row: vers.Row - de.Row}
...
// IsDiagonal distingue les quatre diagonales des quatre orthogonales.
//
...
func (d Direction) IsDiagonal() bool { return d >= NordEst }

// CornerCells renvoie les deux cases orthogonales par lesquelles une
...
func (p Position) CornerCells(d Direction) (Position, Position) {
	v := deplacements[d]
	return Position{p.Column + v.Column, p.Row}, Position{p.Column, p.Row + v.Row}
}

// ChebyshevDistance est le nombre de tours minimal qu'il faut au fugitif pour
...
// ManhattanDistance compte les pas orthogonaux entre deux cases.
//
...
// sign rend -1, 0 ou 1 selon le sens d'un écart.
func sign(n int) int {
	switch {
	case n < 0:
		return -1
