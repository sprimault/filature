...
// comme la sélection.
func troisCoups() []noyau.Coup {
	return []noyau.Coup{
		{Tour: 2, Acteur: noyau.CampFugitif, Type: noyau.CoupDeplacer,
			Depart: noyau.Position{Colonne: 3, Ligne: 3}, Arrivee: noyau.Position{Colonne: 3, Ligne: 2}},
		{Tour: 2, Acteur: noyau.CampFugitif, Type: noyau.CoupDepense, Depense: noyau.DepenseSilence},
		{Tour: 2, Acteur: noyau.CampFugitif, Type: noyau.CoupFinDePhase},
	}
}
...
func TestDecrireChaqueTypeDeCoup(t *testing.T) {
	cas := []struct {
		...
		{noyau.Coup{Type: noyau.CoupPlacer, Acteur: noyau.CampFugitif, Zone: 4}, "sceller la zone 4"},
		{noyau.Coup{Type: noyau.CoupPlacer, Acteur: noyau.CampInspecteurs, Pion: 1,
			Arrivee: noyau.Position{Colonne: 2, Ligne: 6}}, "placer B en (2,6)"},
		{noyau.Coup{Type: noyau.CoupDeplacer, Acteur: noyau.CampInspecteurs, Pion: 0,
			Depart: noyau.Position{Colonne: 1, Ligne: 1}, Arrivee: noyau.Position{Colonne: 2, Ligne: 1}}, "déplacer A"},
		{noyau.Coup{Type: noyau.CoupCapacite, Pion: 2, Capacite: "traqueur"}, "capacité traqueur de C"},
		{noyau.Coup{Type: noyau.CoupDepense, Depense: noyau.DepenseMeurtre}, "dépenser meurtre"},
		{noyau.Coup{Type: noyau.CoupChangerZone, Zone: 1}, "resceller vers la zone 1"},
		{noyau.Coup{Type: noyau.CoupPasser}, "passer"},
		{noyau.Coup{Type: noyau.CoupFinDePhase}, "rendre la main"},
	}
...
func TestDirectionNommee(t *testing.T) {
	depart := noyau.Position{Colonne: 5, Ligne: 5}

	for d, attendu := range map[noyau.Direction]string{
		noyau.Nord:      "nord",
		noyau.Est:       "est",
		noyau.SudOuest:  "sud-ouest",
		noyau.NordOuest: "nord-ouest",
	} {
		coup := noyau.Coup{Type: noyau.CoupDeplacer, Depart: depart, Arrivee: depart.Avance(d)}
