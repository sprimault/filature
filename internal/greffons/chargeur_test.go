...
// manifesteValide rend un greffon de règles minimal mais complet.
...
func TestChargeLeContenuLivre(t *testing.T) {
	...
	for _, cle := range []noyau.Depense{
		noyau.DepenseDoubleDeplacement, noyau.DepenseSilence, noyau.DepenseEffacement,
		noyau.DepenseChangerZone, noyau.DepenseMeurtre,
	} {
...
func TestCleReporteeDepuisLaTable(t *testing.T) {
	...
	for cle, d := range r.Depenses {
		if noyau.Depense(d.Cle) != cle {
			t.Errorf("dépense %s porte la clé %q", cle, d.Cle)
		}
	}
}

// TestGreffonDuDisquePasseParLeMemeChemin vérifie qu'un greffon posé à la main
...
func TestGreffonDuDisquePasseParLeMemeChemin(t *testing.T) {
	...
	r, err := Charger(livres.Livres(), racine)
	if err != nil {
		t.Fatalf("chargement : %v", err)
	}

	if _, connue := r.Capacites["essai"]; !connue {
		t.Error("la capacité du greffon posé sur le disque manque")
	}
...
func TestDossierDeGreffonsAbsent(t *testing.T) {
	r, err := Charger(livres.Livres(), filepath.Join(t.TempDir(), "rien"))
	if err != nil {
...
//
// Deux greffons qui définissent la même capacité doivent le dire : écraser
...
func TestConflitDeCle(t *testing.T) {
	...
	if err == nil {
		t.Fatal("deux greffons définissent la même capacité sans que rien ne le dise")
	}
...
// TestRefus rassemble les manquements de docs/greffons.md §9.
//
// Chacun fait échouer le chargement entier : un greffon à moitié actif est pire
// qu'un greffon absent, et un manifeste écarté en silence se découvre en
// partie.
...
// version.
//
// Deux greffons qui se disent « 1.0.0 » sans être identiques doivent se
...
func TestEmpreinteSurLeContenu(t *testing.T) {
	...
	if a != b {
		t.Error("deux greffons identiques ont des empreintes différentes")
	}
	if a == c {
...
// renommer une forme — laisserait l'empreinte inchangée alors que le greffon
...
// TestValiderRendTousLesManquements vérifie ce que docs/greffons.md attend :
...
func TestValiderAccepteLeContenuLivre(t *testing.T) {
	for _, dossier := range []string{"base", "anglais"} {
		t.Run(dossier, func(t *testing.T) {
			if err := Valider(filepath.Join("..", "..", "greffons", dossier)); err != nil {
...
// ecrire pose un fichier d'essai, avec les droits que gosec exige.
